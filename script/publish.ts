...
const PACKAGE_NAME = "only-my-opencode";
...
console.log("=== Publishing only-my-opencode (multi-package) ===\n");
...
async function updateAllPackageVersions(newVersion: string): Promise<void> {
  ...
  for (const platform of PLATFORM_PACKAGES) {
    const pkgName = `only-my-opencode-${platform}`;
...
async function getContributors(previous: string): Promise<string[]> {
  ...
  const team = ["actions-user", "github-actions[bot]", "fuzen2478"];

  try {
    const compare =
      await $`gh api "/repos/fuzen2478/only-my-opencode/compare/v${previous}...HEAD" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text();
...
async function gitTagAndRelease(
  ...
  await $`git add package.json assets/only-my-opencode.schema.json`;
  for (const platform of PLATFORM_PACKAGES) {
