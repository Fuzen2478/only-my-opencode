...
/**
 * Get the cache directory for only-my-opencode binaries.
...
export function getCacheDir(): string {
  if (process.platform === "win32") {
    ...
    return join(base, "only-my-opencode", "bin");
  }
...
export async function downloadCommentChecker(): Promise<string | null> {
  ...
  console.log(`[only-my-opencode] Downloading comment-checker binary...`);
  ...
  try {
    ...
    console.log(`[only-my-opencode] comment-checker binary ready.`);

    return binaryPath;
  } catch (err) {
    debugLog(`Failed to download: ${err}`);
    console.error(
      `[only-my-opencode] Failed to download comment-checker: ${err instanceof Error ? err.message : err}`,
    );
    console.error(`[only-my-opencode] Comment checking disabled.`);
    return null;
  }
}
