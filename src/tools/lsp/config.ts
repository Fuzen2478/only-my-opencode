...
function getConfigPaths(): { project: string; user: string; opencode: string } {
  ...
  return {
    project: join(cwd, ".opencode", "only-my-opencode.json"),
    user: join(configDir, "only-my-opencode.json"),
...
export function isServerInstalled(command: string[]): boolean {
  ...
  // Runtime wrappers (bun/node) are always available in only-my-opencode context
  if (cmd === "bun" || cmd === "node") {
    return true;
  }
