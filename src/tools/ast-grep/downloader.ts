...
export async function downloadAstGrep(
  ...
  if (!platformInfo) {
    console.error(
      `[only-my-opencode] Unsupported platform for ast-grep: ${platformKey}`,
    );
    return null;
  }
  ...
  console.log(`[only-my-opencode] Downloading ast-grep binary...`);
  ...
  try {
    ...
    console.log(`[only-my-opencode] ast-grep binary ready.`);

    return binaryPath;
  } catch (err) {
    console.error(
      `[only-my-opencode] Failed to download ast-grep: ${err instanceof Error ? err.message : err}`,
    );
    return null;
  }
}
