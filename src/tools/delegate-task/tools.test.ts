...
describe("sisyphus-task", () => {
  ...
  describe("category delegation config validation", () => {
    test("proceeds without error when systemDefaultModel is undefined", async () => {
      ...
      expect(result).not.toContain("only-my-opencode requires a default model");
    });
  });
              ...
              data: [
                {
                  ...
                  parts: [
                    { type: "text", text: "This is the continued task result" },
                  ],
                },
              ],
            }),
    ...
    test("sync mode agent not found returns helpful error", async () => {
      ...
      const mockClient = {
        session: {
          ...
          prompt: async () => {
            throw new Error(
              "Cannot read property 'name' of undefined agent.name",
            );
          },
          messages: async () => ({ data: [] }),
          status: async () => ({ data: {} }),
      ...
      async () => {
        ...
        const mockClient = {
          ...
          session: {
            ...
            status: async () => ({
              data: { ses_writing_gemini: { type: "idle" } },
            }),
          },
        };
