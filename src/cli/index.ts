...
program
  .command("install")
  .description("Install and configure only-my-opencode with interactive setup")
...
Examples:
  ...
  $ bunx only-my-opencode install --no-tui --claude=max20 --openai=yes --gemini=yes --copilot=no
  $ bunx only-my-opencode install --no-tui --claude=no --gemini=no --copilot=yes --opencode-zen=yes
...
program
  ...
  .addHelpText(
    ...
    `
Examples:
  $ bunx only-my-opencode run "Fix the bug in index.ts"
  $ bunx only-my-opencode run --agent Sisyphus "Implement feature X"
  $ bunx only-my-opencode run --timeout 3600000 "Large refactoring task"
...
program
  ...
  .addHelpText(
    ...
    `
Examples:
  $ bunx only-my-opencode get-local-version
  $ bunx only-my-opencode get-local-version --json
  $ bunx only-my-opencode get-local-version --directory /path/to/project
...
program
  .command("doctor")
  .description("Check only-my-opencode installation health and diagnose issues")
...
Examples:
  ...
  $ bunx only-my-opencode doctor --json
...
program
  ...
  .action(() => {
    console.log(`only-my-opencode v${VERSION}`);
  });
