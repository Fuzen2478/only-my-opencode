...
async function runNonTuiInstall(args: InstallArgs): Promise<number> {
  ...
  if (!validation.valid) {
    ...
    console.log();
    printInfo(
      "Usage: bunx only-my-opencode install --no-tui --claude=<no|yes|max20> --gemini=<no|yes> --copilot=<no|yes>",
    );
    console.log();
    return 1;
  }
  ...
  printStep(step++, totalSteps, "Adding only-my-opencode plugin...");
  ...
  printStep(step++, totalSteps, "Writing only-my-opencode configuration...");
  ...
  console.log(`  ${color.dim("gh repo star code-yeongyu/only-my-opencode")}`);
  console.log();
...
export async function install(args: InstallArgs): Promise<number> {
  ...
  s.start("Adding only-my-opencode to OpenCode config");
  const pluginResult = await addPluginToOpenCodeConfig(VERSION);
  if (!pluginResult.success) {
  ...
  s.start("Writing only-my-opencode configuration");
  const omoResult = writeOmoConfig(config);
  if (!omoResult.success) {
  ...
  p.log.message(`  ${color.dim("gh repo star code-yeongyu/only-my-opencode")}`);
