...
export function writeOmoConfig(
  ...
  try {
    ...
    return {
      ...
      error: formatErrorWithSuggestion(err, "write only-my-opencode config"),
    };
  }
}
...
export function detectCurrentConfig(): DetectedConfig {
  ...
  result.isInstalled = plugins.some((p) => p.startsWith("only-my-opencode"));
  ...
  const { hasOpenAI, hasOpencodeZen, hasZaiCodingPlan } =
