...
describe("external-plugin-detector", () => {
  ...
  describe("detectExternalNotificationPlugin", () => {
    ...
    test("should return detected=false when only only-my-opencode is configured", () => {
      // #given - opencode.json with only only-my-opencode
      const opencodeDir = path.join(tempDir, ".opencode");
      fs.mkdirSync(opencodeDir, { recursive: true });
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({ plugin: ["only-my-opencode"] }),
      );
      ...
      expect(result.detected).toBe(false);
      expect(result.pluginName).toBeNull();
      expect(result.allPlugins).toContain("only-my-opencode");
    });
    ...
    test("should detect opencode-notifier plugin", () => {
      ...
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({ plugin: ["only-my-opencode", "opencode-notifier"] }),
      );
    ...
    test("should detect opencode-notifier with version suffix", () => {
      ...
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({
          plugin: ["only-my-opencode", "opencode-notifier@1.2.3"],
        }),
      );
    ...
    test("should detect @mohak34/opencode-notifier", () => {
      ...
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({
          plugin: ["only-my-opencode", "@mohak34/opencode-notifier"],
        }),
      );
